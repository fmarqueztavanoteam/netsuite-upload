		...
		<div id="{{name}}" class="global-views-star-rating-area {{#if isReviewMode}}global-views-star-rating-area-review-mode{{/if}}" data-toggle='rater' data-validation="control" data-name="{{name}}" data-max="{{maxValue}}" data-value="{{value}}">
			...
			{{#if isWritable}}
				<div class="global-views-star-rating-area-writable{{className}}">
					{{#each buttons}}
						<button type="button" data-action="rate" name="{{name}}" value="{{@indexPlusOne}}" title="{{translate 'Rate:'}} {{@indexPlusOne}}" aria-label="{{translate 'Rate:'}} {{@indexPlusOne}}"></button>
					{{/each}}
				</div>
			{{/if}}
			...
			<div class="global-views-star-rating-area-fill" data-toggle='ratting-component-fill' {{#if isViewMode}} style="width: {{filledBy}}%"{{/if}}>
				<div class="global-views-star-rating-area-filled">
					{{#if isReviewMode}}
						{{#each ratedStars}}
							<i class="global-views-star-rating-filled{{className}} {{#unless ../isWritable}}global-views-star-rating-filled-rated-star{{/unless}}" aria-hidden="true"></i>
						{{/each}}
					{{else}}
						{{#each buttons}}
							<i class="global-views-star-rating-filled{{className}}" aria-hidden="true"></i>
						{{/each}}
					{{/if}}
				</div>
			</div>
