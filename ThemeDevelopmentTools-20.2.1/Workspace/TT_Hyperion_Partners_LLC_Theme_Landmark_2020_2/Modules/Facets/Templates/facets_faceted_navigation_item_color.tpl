{{#if showFacet}}
	<div class="facets-faceted-navigation-item-color-facet-group" id="{{htmlId}}" data-type="rendered-facet" data-facet-id="{{facetId}}">
		{{#if showHeading}}
			...
			{{else}}
			<a href="#" class="facets-faceted-navigation-item-color-facet-group-expander {{#if isCollapsed}}collapsed{{/if}}" data-toggle="collapse" data-target="#{{htmlId}}-color-wrapper" data-type="collapse" title="{{facetDisplayName}}">
					<i class="facets-faceted-navigation-item-color-facet-group-expander-icon"></i>
		...
		{{else}}
		<div class="facets-faceted-navigation-item-color-facet-group-wrapper {{#if isCollapsed}} collapse {{else}} in{{/if}}" id="{{htmlId}}-color-wrapper">
		{{/if}}
			<div class="facets-faceted-navigation-item-color-facet-group-content">
				<ul class="facets-faceted-navigation-item-color-picker">
					{{#each displayValues}}
						<li >
							<a href="{{link}}" title="{{label}}" aria-label="{{label}}" data-color="{{label}}" class="facets-faceted-navigation-item-color-option {{#if isLightColor}}white-border{{/if}} {{#if isActive}}active{{/if}}">
								{{#if isImageTile}}
									<img
										src="{{image.src}}"
										alt="{{label}}"
				...
				{{#if showExtraValues}}
					<ul class="facets-faceted-navigation-item-color-picker-extra collapse">
						{{#each extraValues}}
							<li >
								<a href="{{link}}" title="{{label}}" aria-label="{{label}}" data-color="{{label}}" class="facets-faceted-navigation-item-color-extra-option {{#if isLightColor}}white-border{{/if}} {{#if isActive}}active{{/if}}">
									{{#if isImageTile}}
										<img
											src="{{image.src}}"
											alt="{{label}}"
